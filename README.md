...
- `dreamers-full` for the complete pipeline. It accepts a task description, existing plan path(s), or manifest; task mode invokes `dreamers-plan`, while plan path and manifest modes skip planning and use supplied artifacts directly. It runs one automatic triad review per plan, uses Vigil follow-up review reruns by default, gates extra triad/selected-lane reruns for major changes, and keeps gates inline at plan approval, implementation start, templated user testing when triggered, and final pre-PR approval.
