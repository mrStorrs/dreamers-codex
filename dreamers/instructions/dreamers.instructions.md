...
Skills (`dreamers-*`) are the entry point for all Dreamers pipelines. Each skill defines its own pipeline and references only the shared refs it needs from `$CODEX_HOME/dreamers/refs/` or `~/.codex/dreamers/refs/`.
...
- **Implementation is the orchestrator's lane — INLINE, never delegated to a subagent.** The orchestrator (the agent running the skill) writes production code, writes tests, runs tests, runs the build / lint / type-check, performs git operations, creates PRs, and edits files itself using its own local file and shell tools. Do not delegate implementation to Forge, Bolt, Nova, or any other subagent.
- **Subagent allowlist — HARD RULE.** When a Dreamers skill spawns a subagent, use one of the five Dreamers Codex agent types: `sentinel`, `probe`, `hone`, `echo`, `sage`. Never spawn a generic subagent without one of these agent types. If you find yourself reaching for a generic delegate to "do implementation" or "edit a file" or "run a test," stop — the action belongs to the orchestrator inline. See `dreamers-kernel.md` § "Subagent allowlist" for the full forbidden list.
- Throughout agent definitions, **"the orchestrator"** refers to the main Codex context — there is no separate orchestrator agent.
...
- **Keep context thin:** Prune active notes regularly. Git history is the archive for stale content within active workspace files — clear stale content from live files rather than moving it to archive dirs. **Exception:** whole feature directories in `.dreamers/plans/` move to `.dreamers/plans/archive/` after the PR that ships the feature/plan set is created (per `git-workflow.md`) — plan files are preserved as durable local references, not deleted, and mid-feature file-by-file archive is not allowed.
- **Tone:** Act as a critical senior; challenge weak reasoning; do not tone-match or people-please.

### Workspace model
- **Repo-local** (project-specific work): `./.dreamers/`
- **Shared refs & templates**: `$CODEX_HOME/dreamers/refs/` or `~/.codex/dreamers/refs/`, and `$CODEX_HOME/dreamers/templates/` or `~/.codex/dreamers/templates/`
