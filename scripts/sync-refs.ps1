...
.DESCRIPTION
  Refs at dreamers/refs/<NAME>.md are the source of truth. Consumer files
  (skills, agents, and project markdown) declare which refs they want
...
$refsDir = Join-Path $repoRoot 'dreamers/refs'
...
foreach ($f in $refFiles) {
    ...
    $refs[$name] = @{
        Path    = $f.FullName
        RelPath = "dreamers/refs/$($f.Name)"
        Content = (Read-FileText $f.FullName).TrimEnd("`n")
    }
}

# Consumer set: Markdown and TOML files that can inline Dreamers refs, except
# the source refs themselves (a ref containing a marker would create a sync loop).
$consumerRoots = @(
    (Join-Path $repoRoot 'skills'),
    (Join-Path $repoRoot 'agents'),
    (Join-Path $repoRoot 'dreamers/instructions'),
    (Join-Path $repoRoot 'dreamers/templates')
) | Where-Object { Test-Path $_ }

$allConsumers = @()
foreach ($root in $consumerRoots) {
    $allConsumers += Get-ChildItem -Path $root -Include '*.md', '*.toml' -File -Recurse
}
$readme = Join-Path $repoRoot 'README.md'
if (Test-Path $readme) { $allConsumers += Get-Item $readme }

$consumers = $allConsumers | Where-Object { $_.FullName -notmatch '[\\/]dreamers[\\/]refs[\\/]' }
