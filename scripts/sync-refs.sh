...
consumer_roots = [
    repo / "skills",
    repo / "agents",
    repo / "dreamers/instructions",
    repo / "dreamers/templates",
]
consumers: list[Path] = []
for root in consumer_roots:
    if root.exists():
        consumers.extend(sorted(root.rglob("*.md")))
        consumers.extend(sorted(root.rglob("*.toml")))
readme = repo / "README.md"
if readme.exists():
    consumers.append(readme)

for consumer in sorted(set(consumers)):
    rel = consumer.relative_to(repo).as_posix()
    if rel.startswith("dreamers/refs/"):
        continue
