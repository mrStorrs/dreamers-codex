...
## Modes
| Mode | Input | Phase 1 / 1.5 behavior |
|---|---|---|
| 1 | Task description | Invoke `dreamers-plan <task description>` for the Grill phase + right-sized plans → capture plan paths from its output → run Phase 1.5 gate |
...
- Call `update_plan` with a todo list marking all phases at entry. Mode 1: Phase 1 / Phase 1.5 / Phase 2 cycle-N / Phase 3. Modes 2 and 3: Artifact resolution / Phase 2 cycle-N / Phase 3.

## Phase 1 — Planning (Mode 1 only)
- Invoke `dreamers-plan <task description>`. The planning pass must include this phase before proposal approval:
...
- Do not run Phase 1.5 for plan path or manifest mode. After artifact + plan-quality checks pass, proceed directly to Phase 2.
...
- Run the project's type-check + test command (from `AGENTS.md, CODEX.md, or .github/copilot-instructions.md when present`). Fix inline (max 3 attempts) then halt.
...
- Open questions from any reviewer artifact → present each by asking the user; capture; carry decisions into Step 5.
...
- For each gate-triggering finding (or batched group sharing the same refactor scope), ask the user with: reviewer, severity, lens, location, finding, suggested fix, triggered criterion, rationale, breadth estimate. Options: `Apply now` / `Defer — create follow-up plan` / `Other`.
...
- If a reviewer rerun is needed and no major-change trigger applies, spawn `vigil` once with `multi_agent_v1.spawn_agent` using: plan path, changed-files scope, branch/default names, validation commands/results, prior triad artifact paths, and what changed since the triad pass. If multi-agent tools are unavailable, run Vigil inline and preserve the artifact contract. Read the Vigil artifact before applying findings. Route Vigil findings through Step 5.
...
- **INCREMENTAL** (light close-out for this plan):
  ...
  - **Pre-PR approval gate**: present plan summary, validation status, and PR scope. Ask the user to choose Approved/Halt/Other. Halt → emit resume command + stop.
...
- **User approval gate** (MANDATORY, last halt before PR): present milestone summary. ask the user to choose Approved/Halt/Other. Halt → emit resume command + stop.
- Invoke `dreamers-pr` (pass `--issue <#|url>` if the skill input referenced one). Capture PR URL.
