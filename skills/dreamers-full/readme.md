...
```mermaid
flowchart TD
    Start(["dreamers-full the skill input"]) --> ModeCheck{"the skill input<br/>type?"}
...
- **Orange diamonds** — decision points (most call `ask-the-user gate` and surface options to the user).
...
- Step 4 always runs one full `dreamers-review --branch` per plan before PR-bearing code ships. This is the only automatic triad pass.
