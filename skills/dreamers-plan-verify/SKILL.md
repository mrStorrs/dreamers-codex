...
description: "Inline drift check on a plan against current codebase reality. Re-reads the plan, compares against current state, reports drift items. No subagent — orchestrator does it inline. Use when the user asks for dreamers-plan-verify, verify plan, check plan applies, plan drift check."
---
...
The outermost active Dreamers skill owns the plan. If this skill is the entry point, declare via `update_plan`:
- [ ] Read plan file
...
Read the plan file passed as the skill input. If no plan path is provided, halt and ask the user.
