...
```mermaid
flowchart TD
    Start(["dreamers-pr-resolve the skill input"]) --> S1

    S1["Step 1 — Discover open PRs"] --> PRCheck{"PR target?"}
    PRCheck -->|the skill input specified| UseSpecified["Use specified PR"]
    PRCheck -->|"Multiple open<br/>none specified"| PickPR["ask-the-user gate<br/>list open PRs + Other"]
    ...
    S5["Step 5 — Selected-lane review"] --> SpawnLane["Spawn Sentinel required<br/>add Probe/Hone only when needed<br/>scope = files touched by accepts"]
...
- **Selected review lane.** Accepted fixes require Sentinel. Add Probe for coverage/regression risk and Hone for architecture/refactor risk.
