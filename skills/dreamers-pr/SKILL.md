---
name: dreamers-pr
description: "PR creation skill — pushes the current branch, drafts the PR body from pr-description.md template, opens the PR via gh, optionally posts an issue resolution comment. Use when the user asks for dreamers-pr, open the PR, ship the branch."
---
...
- `dreamers/templates/pr-description.md` — PR body shape.

## Codex todo - Before you begin
- Call `update_plan` with a todo list marking all steps at entry: Step 1 / Step 2 / Step 3 / Step 4.
...
## Step 3 — Open the PR
- Read `pr-description.md` template by reading the local file.
...
- Identify the shipped `.dreamers/plans/feature-<slug>/` from the PR body, branch name, commit `Plan:` lines, explicit skill input, or feature directory references.
