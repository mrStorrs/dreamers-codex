...
```mermaid
flowchart TD
    Start(["dreamers-research the skill input"]) --> P1
    ...
    P15["Phase 1.5 — User selection gate"] --> AskUser["ask-the-user gate<br/>multi-select sub-topics<br/>+ Cancel research<br/>+ Other"]
...
- **Parallel where possible.** Phase 2 spawns up to 6 concurrent research/review pairs in batched `multi_agent_v1.spawn_agent` calls.
