...
|---|---|---|
| 1 | Task description | Invoke `dreamers-plan` with the task description for the Grill phase + right-sized plans → capture plan paths from its output → run Phase 1.5 gate |
...
- Read `dreamers/templates/plan-guide-selector.md`, then read only the matching guide for each plan.
...
- Before reading `.dreamers/` files, resolve and read `dreamers-kernel.md` and `git-workflow.md` per the Codex runtime mapping, including startup verification.

## Phase 1 — Planning (Mode 1 only)
- Invoke `dreamers-plan` with the task description in the same orchestrator context.
...
- Open questions from any reviewer artifact → present each to the user; capture; carry decisions into Step 5.
...
- For each gate-triggering finding (or batched group sharing the same refactor scope), ask the user with: reviewer, severity, lens, location, finding, suggested fix, triggered criterion, rationale, breadth estimate. Options: `Apply now` / `Defer — save to defered.md` / `Other`.
